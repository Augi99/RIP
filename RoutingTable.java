...
public class RoutingTable {
    ...
    public boolean checkIfDuplicate(Entry entry)
    {
        for(int i = 0; i < num; i++)
        {
            if(entry.getDestinationNetwork().getAddress().equals(content[i].getDestinationNetwork().getAddress()))
            {
                return true;
            }
        }
